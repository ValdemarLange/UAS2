...
class Observation:
    ...
    def __repr__(self):
        return repr(
            "Observation - point %d - camera %d (%f %f)"
            % (
                self.point_id,
                self.camera_id,
...
class VisualSlam:
    def __init__(self, inputdirectory, feature="ORB"):
        ...
        self.bfPermissive = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=False)
    ...
    def set_camera_matrix(self):
        ...
        self.camera_matrix = np.array(
            [
                [7.070912e02, 0.0e00, 6.018873e02],
                [0.000000000000e00, 7.070912000000e02, 1.831104000000e02],
    ...
    def initialize_map(self, image_pair):
        ...
        ic(ip.R)
        ic(ip.t)
        first_camera = TrackedCamera(np.eye(3), np.zeros((3)), ip.frame1.id, fixed=True)
        second_camera = TrackedCamera(ip.R, ip.t.T[0], ip.frame2.id, fixed=False)
        first_camera = self.map.add_camera(first_camera)
        second_camera = self.map.add_camera(second_camera)

        for match in ip.matches_with_3d_information:
            max_distance_requirements = 70
            if np.linalg.norm(match.point) > max_distance_requirements:
                # print(match.point)
                continue
            tp = TrackedPoint(
    ...
    def add_new_match_to_map(self, match, first_camera, second_camera):
        tp = TrackedPoint(match.point, match.descriptor1, match.color, match.featureid1)
        tp = self.map.add_point(tp)
    ...
    def estimate_current_camera_position(self, current_frame):
        ...
        descriptors_from_current_frame = [
            feature.descriptor for feature in current_frame.features
        ]

        descriptors_in_map = [
            point.descriptor
            for point in self.map.points
            if np.linalg.norm(point.point) < 200
        ]
        ...
        try:
            camera_pose = self.map.cameras[-1].pose()
            list_of_matches_with_reprojection_errors = []
            for point_3d, image_coord in zip(map_coords, image_coords):
                error = self.map.calculate_reprojection_error_for_point(
                    np.hstack((point_3d, 1)), camera_pose.copy(), image_coord
                )
                print("reprojection error of point: ", error)
                if error < 200:
                    list_of_matches_with_reprojection_errors.append(
                        (point_3d, image_coord, error)
                    )

            map_coords_temp, image_coords_temp, _ = zip(
                *list_of_matches_with_reprojection_errors
            )

            # TODO:
            # This appears to be a soft spot, as the position estimation often fails.
            last_camera_tvec = self.map.cameras[-1].t.copy()
            last_camera_rvec, _ = cv2.Rodrigues(self.map.cameras[-1].R)
            print("cv2.solvePnPRefineLM")
            rvec, tvec = cv2.solvePnPRefineLM(
                np.array(map_coords_temp),
                np.array(image_coords_temp),
                self.camera_matrix,
                np.zeros(4),
                last_camera_rvec.copy(),
                last_camera_tvec.copy(),
            )
            print("rvec")
            print(rvec)
            print(tvec)
            ...
            for point_3d, image_coord, _ in list_of_matches_with_reprojection_errors:
                error = self.map.calculate_reprojection_error_for_point(
                    np.hstack((point_3d, 1)), camera_pose, image_coord
                )
                print("reprojection error of point: ", error)
            ...
            if retval:
                ...
                print(tvec)
                R, _ = cv2.Rodrigues(rvec)
                camera = TrackedCamera(R, tvec, current_frame.id, fixed=False)
                ...
                for idx in inliers[:, 0].tolist():
                    ...
                    observation = Observation(
                        tp.point_id, camera.camera_id, match.imagecoord
                    )
                    self.map.observations.append(observation)
    ...
    def match_current_and_previous_frame(self):
        ...
        # self.three_dim_viewport.paint(self.map.points, self.map.cameras)
    ...
    def run(self):
        ic(self.input_directory)
        list_of_files = glob.glob("%s" % self.input_directory)
        list_of_files.sort()
...
vs = VisualSlam(r"../../08_visual_odometry_miniproject/input/frames_limited/*.jpg")
