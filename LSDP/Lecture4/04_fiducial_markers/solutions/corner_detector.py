...
class ChessBoardCornerDetector:
    ...
    def detect_chess_board_corners(self, img, debug=False, *, path_to_image=None, path_to_output_folder=None) -> Tuple[Dict, int, List]:
        try:
    ...
    @staticmethod
    def extract_corners_and_image_points(calibration_points):
        corners = []
        image_points = []
        for xindex, xvals in calibration_points.items():
            for yindex, yval in xvals.items():
                corners.append([xindex, yindex, 0, 1])
                image_points.append([yval[0], yval[1]])
        corners = np.array(corners)
        image_points = np.array(image_points)
        return corners, image_points
