...
def main():
    ...
    tracker = MarkerTracker.MarkerTracker(order=4, kernel_size=91, scale_factor=0.01)
    ...
    cv2.imwrite(
        "hubsan_magnitude_response_n4_kernel.png", magnitude / maximum_magnitude * 255
    )
    cv2.imwrite(
        "hubsan_magnitude_response_inverted_n4_kernel.png",
    ...
    cv2.imwrite(
        "hubsan_region_around_detected_marker.png", region_around_detected_marker
    )
    ...
    merged_input_and_quality_template = cv2.addWeighted(
        region_around_detected_marker.astype("float64"),
        0.5,
        quality_template * 255,
        0.5,
        1,
    )
    cv2.imwrite(
        "hubsan_merged_input_and_oriented_quality_template_oriented.png",
        merged_input_and_quality_template,
    )


def image_of_real_and_imag_part_of_kernel():
    tracker = MarkerTracker.MarkerTracker(order=4, kernel_size=101, scale_factor=1)
    ...
    kernel_argument_hsv = np.dstack(
        (
            kernel_argument * 180 / 3.1415926535 + 180,
            kernel_argument * 0 + 1,
            kernel_argument * 0 + 255.0,
        )
    )
...
def image_of_quality_templates():
    tracker = MarkerTracker.MarkerTracker(order=4, kernel_size=101, scale_factor=0.01)
    ...
    cv2.imwrite(
        "quality_template_oriented.png", (0.5 + bright_regions - dark_regions) * 255
    )
    ...
    cv2.imwrite(
        "quality_template_plain.png", (0.5 + bright_regions - dark_regions) * 255
    )
