...
class VisualSlam:
    def __init__(self, inputdirectory, skip_gui=False, feature="ORB"):
    ...
    def set_camera_matrix(self):
        # TODO: Update with calibrated camera
        ...
        #     [
        #         [1750, 0.0, 1920 / 2 - 10],
        #         [0.000000000000e00, 1750, 1080 / 2 + 32],
        #         [0.000000000000e00, 0.000000000000e00, 1.000000000000e00],
        #     ]
        # )

        # Mavic 3 Multispectral camera calibration based on this dataset
        # Nextcloud/Shared/2026/2026-04-09 Mavic 3 camera calibration video/
        self.camera_matrix = np.array(
            [
                [1348.78136721, 0.0, 960.57640716],
                [0.0, 1348.57759941, 535.73536774],
                [0.0, 0.0, 1.0],
            ]
        )
    ...
    def initialize_map(self, image_pair):
        ...
        second_camera = TrackedCamera(
            ip.R, ip.t.T[0], ip.frame2.id, image_pair.frame2.image, fixed=True
        )
    ...
    def estimate_camera_position_in_map(self):
        ...
        print("Matches with map: ", len(matches_with_map))
    ...
    def find_matches_between_current_frame_and_map(self) -> list[MatchWithMap]:
        ...
        # Suggestion to look at self.bf.knnMatch as it might be faster
    ...
    def match_current_and_previous_frame(self):
        ...
        self.unfreeze_cameras(2)
        self.print_camera_details()
        self.map.limit_number_of_camera_in_map(15)
        cv2.waitKey(20)
...
parser.add_argument(
    "--skip_gui",
    action="store_true",
    help="if the option is given no gui elements are shown",
)
args = parser.parse_args()

vs = VisualSlam(args.directory, args.skip_gui)
