...
class Map:
    ...
    def remove_observations_with_reprojection_errors_above_threshold(
        self, threshold=100
    ):
        camera_dict = {}
    ...
    def calculate_reprojection_error(self, threshold=10):
        ...
        for observation in self.observations:
            ...
            sqerror = np.abs(dx * dx) + np.abs(dy * dy)
            if sqerror > threshold:
                # print("high reprojection error: %f" % sqerror[0])
                # print(observation)
                pass
            total_error += sqerror[0]
    ...
    def optimize_map(self, postfix=""):
        ...
        self.update_camera_poses(camera_vertices)
        self.update_point_coordinates(point_vertices)

    def update_camera_poses(self, camera_vertices):
    ...
    def update_point_coordinates(self, point_vertices):
    ...
    def remove_camera_from_map(self, camera_to_remove):
        ...
        list_of_camera_ids = set([camera.camera_id for camera in self.cameras])
        ...
        self.points = [
            point for point in self.points if point.point_id not in points_to_remove
        ]

        list_of_point_ids = set([point.point_id for point in self.points])
    ...
    def show_map_statistics(self):
        ...
        if len(self.observations) > 0:
            print(
                ...
                % (self.calculate_reprojection_error() / len(self.observations))
            )
    ...
    def show_observation_matrix(self):
        ...
        obs_matrix = collections.defaultdict(lambda: collections.defaultdict(lambda: 0))
        for point in self.points:
            cameras_that_see_point = [
                observation.camera_id
                for observation in observations_of_point[point.point_id]
            ]
            combinations = [
                (cam1, cam2)
                for cam1 in cameras_that_see_point
                for cam2 in cameras_that_see_point
                if cam1 < cam2
            ]
